...
from adaptive_vr.pipeline import get_direct_response, get_response_with_attention
from adaptive_vr.qwen_vl_model import AdaptiveVRQwenVLModel
...
if __name__ == "__main__":
    ...
    parser.add_argument("--annotation_path", type=str, default="adaptive_vr/adaptive_vr_data", help="Path to benchmark annotations")
    ...
    if args.answers_file is None:
        answers_dir = f"adaptive_vr/eval/answers/{benchmark}"
        answers_dir = os.path.join(answers_dir, os.path.basename(args.model_path))
        os.makedirs(answers_dir, exist_ok=True)
        answer_tag = "adaptive_vr" if not args.direct_answer else "direct_answer"
    ...
    base_model = AdaptiveVRQwenVLModel(
    ...
    ic_examples_path = f"adaptive_vr/ic_examples/{benchmark}.json"
    ...
    print(f"Entered adaptive pipeline : {pipeline_called_count}")
