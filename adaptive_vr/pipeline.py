...
from adaptive_vr.base_model import AdaptiveVRModel
from adaptive_vr.qwen2_5_methods import rel_attention_qwen2_5
from adaptive_vr.utils import *


def get_response_with_attention(
    base_model: AdaptiveVRModel,
    annotation,
    ic_examples,
    ...
    targets = base_model.generate_visual_cues_using_ic(ic_examples, question)
    ...
    first_avg_conf, _, _, _ = base_model.free_form_using_nodes(
        image_pil,
    ...
    if first_avg_conf < conf_threshold:
        ...
        images_list, attention_maps, _, _ = process_attention_to_image_list(base_model, image_pil, targets)
    ...
    for option_str in options:
        ...
        final_output = base_model.free_form_using_nodes(
            image_pil,
            question_input,
            images_list,
...
def process_attention_to_image_list(base_model: AdaptiveVRModel, image_pil, targets, include_original=False):
    if not targets:
    ...
    if union_bbox:
        ...
        crop_image = base_model.resize_image(crop_image)
...
def get_direct_response(
    ...
    for option_str in options:
        question_input = format_question(question, option_str)
        answers.append(base_model.free_form_using_nodes(image_pil, question_input, image_list))
    return answers
