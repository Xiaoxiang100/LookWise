...
from lookwise.llava.constants import IMAGE_TOKEN_INDEX
import numpy as np
...
def process_images_with_multi_ratio(images, image_processor, ratios, model_cfg,show_crop=False):
    if len(ratios) > len(images):
        ratios = ratios[:len(images)]
    elif len(ratios) < len(images):
        ratios = ratios + [ratios[-1]] * (len(images)-len(ratios))
    new_images = []
    for image, ratio in zip(images, ratios):
        if ratio == "highres":
            image = process_highres_image(image, image_processor, model_cfg.image_grid_pinpoints)
        elif ratio == "anyres" or "anyres_max" in ratio:
            image = process_anyres_image(image, image_processor, model_cfg.image_grid_pinpoints, show_crop)
        elif ratio == "pad":
            image = expand2square(image, tuple(int(x * 255) for x in image_processor.image_mean))
            image = image_processor.preprocess(image, return_tensors="pt")["pixel_values"][0]
