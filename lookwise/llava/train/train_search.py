...
from lookwise.llava.constants import IGNORE_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN, IMAGE_TOKEN_INDEX, DEFAULT_OBJECT_TOKEN
from lookwise.llava.train.llava_trainer import LLaVATrainer

from lookwise.llava import conversation as conversation_lib
from lookwise.llava.model import *
from lookwise.llava.mm_utils import process_highres_image, process_anyres_image, process_highres_image_crop_split, tokenizer_image_token, get_patch
from lookwise.llava.utils import rank0_print, process_video_with_pyav, process_video_with_decord
    ...
    for source in sources:
        for sentence in source:
            ...
            if DEFAULT_OBJECT_TOKEN in sentence["value"]:
                ...
                arr = sentence["value"].split(DEFAULT_OBJECT_TOKEN)
                res = ""
                for i in range(num):
                    res += (arr[i] + object_str_list[i])

                sentence["value"] = res + arr[-1]
    return sources
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0]:
            ...
            assert type(image_file) is str
            image_file = image_file.replace("coco2014/", "coco/").replace("coco2017", "coco/")
            ...
            if is_search:
                ...
                for target_instance in target_instances:
                    ...
                    # FIXME
                    resized_bbox = get_patch(bbox, image_width, image_height, patch_size=384, patch_scale=None)
                    bbox_list.append(resized_bbox)
                    image_patch = image.crop(resized_bbox)
                    image_patches.append(image_patch)
                bbox_list = [self.normalize_bbox(bbox, image.width, image.height) for bbox in bbox_list]
                object_str_list = []
                for bbox in bbox_list:
                    object_str_list.append("a sub-patch {} at location [{:.3f},{:.3f},{:.3f},{:.3f}]".format(DEFAULT_IMAGE_TOKEN, bbox[0], bbox[1], bbox[2], bbox[3]))
            image = [self.process_image(image)] + [self.process_image(p, 'pad') for p in image_patches]

            sources = preprocess_multimodal(copy.deepcopy([e["conversations"] for e in sources]), self.data_args, object_str_list=object_str_list)
        else:
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        rank0_print("model.config.image_grid_pinpoints:", model.config.image_grid_pinpoints)
