...
ANNO_PATH="${ANNO_PATH:-/your/data/lookwise_data}"
BENCHMARK="hr-bench_8k"
CONF_THRESHOLD="${CONF_THRESHOLD:-1.00}"

MODE_FLAG=""
MERGE_FILE_SUFFIX="_lookwise"
if [[ "$1" == "direct" ]]; then
...
ANSWERS_DIR="lookwise/eval/answers/${BENCHMARK}/${MODEL_BASENAME}"

mkdir -p "${ANSWERS_DIR}"
echo "========================================="
echo "  LookWise evaluation"
echo "========================================="
...
for IDX in $(seq 0 $((CHUNKS-1))); do
    CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} python lookwise/eval/run_inference.py \
...
MERGE_FILE="${ANSWERS_DIR}/hr-bench_8k_test_threshold${MERGE_FILE_SUFFIX}_th${THRESHOLD_STR}.jsonl"
...
echo "Score with:"
echo "python lookwise/eval/eval_results_hr-bench.py --answers-file ${MERGE_FILE}"
echo "========================================="
